...
class UMAP_Util():
    ...
    def __init__( 
        ...
        self.centroids = [] #high dim centroids - do i even need this? 
    ...
    def train( self, data_points : torch.Tensor):
        ...
        data_points_np = data_points.numpy() #convert data to np array to run umap

        #embedding_np = self.reducer.fit_transform(data_points_np) #flattened data

        #self.embedding = torch.tensor(embedding_np) #storing the 2d data as a tensor is this needed?
        #print(self.embedding)

        self.kmeans.fit(embedding_np)#kmeans on flattened data
        ...
        for i in range(0, self.kmeans.cluster_centers_.shape[0]):
            ...
            self.centroids.append(tensor_mean) #add high dim mean to centroids
        '''
